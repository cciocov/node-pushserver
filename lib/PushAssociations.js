...
var add = function (user, deviceType, token, channels) {
    PushAssociation.update({user: user, token: token}, {user: user, type: deviceType, token: token, channels: channels}, {upsert: true}, function(err) {
...
var getAll = function (channels, callback) {
    var wrappedCallback = outputFilterWrapper(callback);

    if (channels && channels.length) {
      PushAssociation.where('channels')
          .in(channels)
...
var getForUsers = function (users, channels, callback) {
    var wrappedCallback = outputFilterWrapper(callback);
...
var outputFilterWrapper = function (callback) {
    return function (err, pushItems) {
        ...
        var items = _.map(pushItems, function (pushItem) {
            return _.pick(pushItem, ['user', 'type', 'token', 'channels'])
        });
